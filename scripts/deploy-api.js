...
// gigio-fix
const source = `gs://${env.PKG_BUCKET}/${pkg.name}_${version}.zip`;
const dbSocketPath = env.DB_SOCKET_PATH || "/cloudsql";
const serverName =
  (env.PGSERVERNAME || "").indexOf(region) === -1
...
const envVars = [
  ...
  `PGHOST=${dbSocketPath}/${serverName}`,
  `PGUSER=${env.PGUSER}`,
